 ...
 * @param {{highlightClassName: string; referenceClassName: string, hyperlinksClassName: string}} options
 * @returns {JSXElement}
 */
export function highlightSections(
  { text, reference, hyperlinks },
  { highlightClassName, referenceClassName, hyperlinksClassName }
) {
  const jsxElems = [];

  if (text) {
    jsxElems.push(...getMarkElems(text, highlightClassName));
  }

  if (reference) {
    jsxElems.push(
      <section className={referenceClassName} key="reference-section">
        {getMarkElems(reference, highlightClassName)}
      </section>
    );
  }
  ...
  if (hyperlinks && hyperlinks.length) {
    jsxElems.push(
      ...hyperlinks.map(({ title, summary }, idx) => (
        <section
          className={hyperlinksClassName}
          key={`hyperlink-section-${idx}`}
        >
          {getMarkElems(`${title || ''} ${summary || ''}`, highlightClassName)}
        </section>
      ))
    );
  }
