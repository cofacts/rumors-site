...
function ComponentInput({ pieceType, type, anchorEl, selected, viewBox, title, onChange = () => {} }) {
        ...
        {options.map((value) => (        
          <MenuItem key={value} value={value} className={classes.menuItem}>
            <svg className={classes.avatarPiece} viewBox={viewBox}>
              {value && pieceType && React.createElement(pieceType[value])}
            </svg>
          </MenuItem>
        ))}
      </TextField>
  ...
  return (
    <>
      ...
      >
        {options.map((value) => (        
          <MenuItem key={value} value={value} className={classes.menuItem}>
            <span className={classes.colorGird} style={{background: value}}></span>
          </MenuItem>
        ))}
      </TextField>
    </>
