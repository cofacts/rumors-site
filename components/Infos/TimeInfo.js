...
function formatTimeInfoDate(date) {
  const now = new Date();
  const rtf = new Intl.RelativeTimeFormat(process.env.LOCALE, { style: 'narrow' });
  const dtf = new Intl.DateTimeFormat(process.env.LOCALE, { dateStyle: 'full', timeStyle: 'long' });
