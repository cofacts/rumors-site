...
const LinkAutherList = ({articleId}) => (<Link
  href={{
    pathname: '/articles',
    query: {
...
><a className="link-auther">查看該用戶回報的所有文章
  <style jsx>{`
    .link-auther {
      color: #5e7d8f;
      align-self: flex-end;
...
class ReplyRequestReason extends PureComponent {
  ...
  renderBlockReason = ()=>{
    const { replyRequest, voteReplyRequest } = this.props;
    ...
    <div className="box-vote">
          ...
          >
            <span className="vote-num">{replyRequest.get("positiveFeedbackCount")}</span>
            <svg
          ...
          >
            <span className="vote-num">{replyRequest.get("negativeFeedbackCount")}</span>
            <svg
        ...
        <style jsx>{`
          ...
          .btn-up-vote:not(:disabled) .icon:hover,.btn-up-vote:disabled .icon{
            fill: #6dc00c;
          }
          .btn-down-vote:not(:disabled) .icon:hover,.btn-down-vote:disabled .icon{
            fill: red;
          }
